    ...
    <main id="top" className="bg-ivory text-ink">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-6 lg:grid-cols-[1fr_1.05fr]">
          <Reveal>
            ...
            <div className="mt-9 flex flex-wrap items-center gap-4">
              ...
              <a href="#proceso" className="btn-line">
                Cómo lo hacemos
              </a>
            </div>
          ...
          <Reveal delay={150} className="relative">
            <div className="img-zoom relative overflow-hidden rounded-[2rem] rounded-tr-[6rem]">
              <img
                src={images.hero}
                alt="Ambiente de sala con muebles de autor en madera"
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {colecciones.map((c, i) => (
            <Reveal key={c.name} delay={i * 90}>
              <a href="#contacto" className="img-zoom group relative block overflow-hidden rounded-2xl">
      ...
      <section id="galeria" className="mx-auto max-w-7xl px-6 py-24">
        <Reveal className="mb-14 text-center">
          ...
          <h2 className="mt-3 font-display text-4xl font-medium text-espresso sm:text-5xl">
            Piezas que ya encontraron su hogar
          </h2>
        </Reveal>

        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:gap-6">
          {images.galeria.map((src, i) => (
            <Reveal key={src} delay={(i % 3) * 90} className={i % 5 === 0 ? "row-span-2" : ""}>
              <div className="img-zoom group relative h-full overflow-hidden rounded-2xl">
                <img
                  src={src}
                  alt={`Pieza de mobiliario ${i + 1}`}
                  className={`w-full object-cover ${i % 5 === 0 ? "h-full min-h-[400px]" : "h-64"}`}
                  loading="lazy"
                />
                <div className="absolute inset-0 flex items-end bg-gradient-to-t from-espresso/60 to-transparent p-5 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
      ...
      <section className="mx-auto max-w-7xl px-6 py-24">
        <Reveal className="mb-14 text-center">
          ...
          <h2 className="mt-3 font-display text-4xl font-medium text-espresso sm:text-5xl">
            Quienes ya viven con nuestras piezas
          </h2>
        </Reveal>
          ...
          {testimonios.map((t, i) => (
            <Reveal key={t.name} delay={i * 90}>
              <figure className="flex h-full flex-col rounded-2xl border border-espresso/10 bg-linen/50 p-8">
                ...
                <blockquote className="mt-4 flex-1 font-display text-xl italic leading-relaxed text-espresso">
                  “{t.text}”
                </blockquote>
          ...
          <div className="grid gap-12 lg:grid-cols-[1fr_1fr]">
            <div>
              ...
              <p className="mt-5 max-w-md text-ivory/75">
                Diseño sin costo para tu proyecto a la medida. Respondemos en menos de 24
                horas.
              </p>
            ...
            <form
              className="grid gap-4 rounded-3xl bg-ivory p-7 text-ink"
            ...
            >
              ...
              <select className="rounded-xl border border-espresso/15 bg-white px-4 py-3 text-sm text-ink/70 outline-none transition focus:border-walnut">
                ...
                <option>Cocina / Carpintería fija</option>
              </select>
      ...
      <footer className="border-t border-espresso/10 bg-ivory">
        <div className="mx-auto max-w-7xl px-6 py-14">
          <div className="grid gap-10 md:grid-cols-[1.6fr_1fr_1fr]">
            ...
            {[
              { h: "Explorar", items: ["Colecciones", "Portafolio", "Fabricación", "El taller"] },
              { h: "Contacto", items: ["Cotizar proyecto", "Showroom", "Preguntas frecuentes", "Garantía"] },
            ].map((col) => (
              <div key={col.h}>
                <h4 className="kicker text-espresso/70">{col.h}</h4>
                <ul className="mt-5 space-y-3">
                  {col.items.map((it) => (
                    <li key={it}>
                      <a href="#" className="text-sm text-ink/60 transition hover:text-espresso">{it}</a>
                    </li>
                  ))}
                </ul>
              </div>
